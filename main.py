...
if __name__ == '__main__':
    ...
    def inference(args, epoch, data_loader, model, offset=0):
        ...
        statistics = []
        total_loss = 0
        for batch_idx, (data, target, flo_filenames, flo_rgb_filenames) in enumerate(progress):
            flo_filenames = flo_filenames[0]
            flo_rgb_filenames = flo_rgb_filenames[0]
            ...
            if args.save_flow or args.render_validation:
                for i in range(args.effective_inference_batch_size):
                    ...
                    flow_utils.writeFlow(flo_filenames[i], _pflow)
                    flow_utils.writeFlowRGB(flo_rgb_filenames[i], _pflow)
